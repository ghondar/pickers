...
Version `v3` for @ghondar/pickers is coming with a whole bunch of breaking changes. This is a guide of how-to update :)
<Ad />

#### ⬇️ Install

First of all, you need to install the new package, because the project is transferred to mui-org!
Please also note, that **v3** of `@ghondar/pickers` relies on **v4** of `@material-ui/core`

```
npm i @ghondar/pickers @material-ui/core@latest

yarn add @ghondar/pickers @material-ui/core@latest
```
...
```diff
- import { InlineDatePicker } from '@ghondar/pickers'
+ import { DatePicker } from '@ghondar/pickers'
